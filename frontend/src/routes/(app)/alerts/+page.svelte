<script lang="ts">
  ...
  // Svelte 5: Automatically reload whenever the toggle state changes
  $effect(() => {
    loadAlerts();
  });
  ...
  async function unresolve(id: number) {
    removing = new Set([...removing, id]);
    try {
      const res = await fetch(`/api/v1/alerts/${id}/unresolve`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token()}` },
      });
      if (res.ok) {
  ...
  // When checkbox is ON → show only resolved. When OFF → show only active.
...
<div class="space-y-5">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div class="flex items-center gap-3">
      <div
        class="p-2.5 rounded-xl {activeCount > 0
          ? 'bg-red-100 dark:bg-red-500/20'
          : 'bg-emerald-100 dark:bg-emerald-500/20'}"
      >
        <Bell
          class="w-6 h-6 {activeCount > 0
      ...
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">
          {#if unreadCount > 0}
            ...
            {#if activeCount !== unreadCount}
              · {activeCount} total active{/if}
    ...
    <div class="flex flex-wrap items-center gap-2">
      <label
        class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 cursor-pointer select-none px-2"
      >
        <input
          type="checkbox"
  ...
  {#if isLoading}
    <div class="space-y-3">
      {#each [1, 2, 3] as _}
        <div
          class="h-20 bg-gray-100 dark:bg-gray-800/50 rounded-xl animate-pulse border border-gray-200 dark:border-gray-800"
        ></div>
      {/each}
    </div>
  {:else if alerts.length === 0}
    <Card>
      <div class="text-center py-16">
        <div
          class="w-20 h-20 bg-emerald-100 dark:bg-emerald-500/20 rounded-full flex items-center justify-center mx-auto mb-5 ring-4 ring-emerald-100 dark:ring-emerald-500/10"
        >
        ...
        <p class="text-gray-500 dark:text-gray-400 max-w-sm mx-auto">
          {showResolved
            ...
            : "No active alerts. The system is running normally."}
        </p>
      </div>
    </Card>
  {:else if displayedAlerts.length === 0}
    <Card>
      <div class="text-center py-16">
        ...
        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">
          {showResolved ? "No resolved alerts yet." : "All Clear! 🎉"}
        </h3>
        <p class="text-gray-500 dark:text-gray-400 max-w-sm mx-auto">
          {showResolved
    ...
    <div class="space-y-2.5">
      {#each displayedAlerts as alert (alert.id)}
        ...
        <div
          ...
          onkeydown={(e) =>
            e.key === "Enter" && !alert.is_read && markRead(alert.id)}
        >
          <!-- Unread dot -->
          {#if !alert.is_read}
            <span
              class="absolute top-4 right-[52px] w-2 h-2 bg-red-500 rounded-full animate-pulse"
            ></span>
          {/if}
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-0.5 flex-wrap">
                <span
                  class={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${cfg.badge}`}
                >
                  {alert.severity}
                </span>
            ...
            {#if !alert.is_resolved}
              ...
              >
                {#if isRemoving}
                  <span
                    class="w-4 h-4 border-2 border-current/30 border-t-emerald-500 rounded-full animate-spin block"
                  ></span>
                {:else}
                  <Check class="w-4 h-4" />
                {/if}
            ...
            {:else}
              <button
                ...
                disabled={isRemoving}
                title="Unresolve"
                class="shrink-0 mt-0.5 w-8 h-8 flex items-center justify-center rounded-lg text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-100 transition-all disabled:opacity-50"
              >
                {#if isRemoving}
                  <span
                    class="w-4 h-4 border-2 border-current/30 border-t-emerald-500 rounded-full animate-spin block"
