...
<div class="space-y-6 relative">
  ...
  >
    <div>
      <h1
        class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white mb-2 flex items-center transition-colors"
      >
        <ShieldAlert class="w-8 h-8 mr-3 text-red-500" /> Administration
      </h1>
      <p class="text-gray-600 dark:text-gray-400 transition-colors">
        System-level management. Restricted to admins.
      </p>
    </div>
    <Button
      variant="primary"
  ...
  <!-- Tabs -->
  <div
    class="flex border-b border-gray-200 dark:border-gray-800 transition-colors"
  >
    <button
      onclick={() => (activeTab = "users")}
      class={`px-4 py-3 font-medium text-sm flex items-center transition-colors ${activeTab === "users" ? "border-b-2 border-blue-500 text-blue-600 dark:text-blue-400" : "text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"}`}
    >
      <Users class="w-4 h-4 mr-2" /> Users & Roles
    </button>
    <button
      onclick={() => (activeTab = "buildings")}
      class={`px-4 py-3 font-medium text-sm flex items-center transition-colors ${activeTab === "buildings" ? "border-b-2 border-blue-500 text-blue-600 dark:text-blue-400" : "text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"}`}
    >
    ...
    <Card>
      <div class="flex justify-between items-center mb-6">
        ...
        <input
          ...
          class="bg-gray-50 dark:bg-black/50 border border-gray-200 dark:border-gray-800 rounded-md px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-all text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
        />
      </div>
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr
              class="border-b border-gray-200 dark:border-gray-800 text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider"
            >
          ...
          <tbody class="text-sm divide-y divide-gray-200 dark:divide-gray-800">
            {#each filteredUsers as user}
              ...
              >
                ...
                <td class="py-3 px-4">
                  <span
                    class={`px-2 py-1 rounded-full text-xs font-semibold border ${getRoleBadgeStyles(user.role)}`}
                  >
                ...
                <td class="py-3 px-4">
                  <span
                    class={`px-2 py-1 rounded-full text-xs font-semibold ${user.is_active ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"}`}
                  >
                ...
                <td class="py-3 px-4 text-right">
                  <button
                    ...
                    class="inline-flex items-center gap-1.5 text-blue-500 hover:text-blue-700 dark:hover:text-blue-300 font-medium text-sm transition-colors hover:bg-blue-50 dark:hover:bg-blue-500/10 px-2 py-1 rounded-md"
                  >
            ...
            {:else}
              <tr>
                <td
                  ...
                  >No users found matching your search.</td
                >
              </tr>
            {/each}
          </tbody>
  ...
  {:else if activeTab === "buildings"}
    <Card>
      <p class="text-gray-500 dark:text-gray-400 py-10 text-center">
        In development — manage buildings and apartments.
      </p>
    </Card>
  {/if}
</div>
      ...
      <form onsubmit={handleAddUser} class="p-6 space-y-4">
        ...
        >
          ...
          <button
            type="submit"
            disabled={isAdding}
            class="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-lg transition-colors shadow-sm"
          >
      ...
      <form onsubmit={handleEditUser} class="p-6 space-y-4">
        ...
        <div>
          <label
            ...
            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
          >
            New Password <span class="text-gray-400 font-normal"
