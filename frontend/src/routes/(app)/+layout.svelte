...
<div class="flex h-screen bg-gray-50 text-gray-900 dark:bg-black dark:text-gray-100 transition-colors duration-300">
    ...
    <div class="mt-auto border-t border-gray-200 dark:border-gray-800 p-4 shrink-0 overflow-hidden">
      {#if currentUser}
        <div class="flex items-center space-x-3 w-full">
          <div class="w-8 h-8 rounded-full bg-linear-to-tr from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold shrink-0">
  ...
  >
    <div class="h-16 flex items-center justify-between px-6 border-b border-gray-200 dark:border-gray-800 shrink-0">
      ...
      <button onclick={closeMobileMenu} class="p-2 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors">
        <X class="w-6 h-6" />
      </button>
    </div>
      ...
      {#if currentUser}
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            <div class="w-10 h-10 rounded-full bg-linear-to-tr from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold">
              {currentUser.full_name ? currentUser.full_name[0].toUpperCase() : currentUser.email[0].toUpperCase()}
            </div>
            <div>
              <p class="text-sm font-semibold text-gray-900 dark:text-white">{currentUser.full_name || "User"}</p>
              <p class="text-[10px] uppercase font-bold text-blue-500">{currentUser.role.replace("_", " ")}</p>
            </div>
          </div>
          <button onclick={handleLogout} class="p-2 text-gray-400 hover:text-red-500 transition-colors">
  ...
  <div class="flex-1 flex flex-col overflow-hidden">
    <!-- Top Header -->
    <header class="h-16 border-b border-gray-200 bg-white/60 dark:border-gray-800 dark:bg-gray-900/40 backdrop-blur flex items-center justify-between px-4 md:px-6 shrink-0">
      <button onclick={() => (isMobileMenuOpen = true)} class="md:hidden p-2 -ml-2 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors">
        <Menu class="w-6 h-6" />
      </button>
    ...
    <!-- Page Content -->
    <main class="flex-1 overflow-y-auto p-4 md:p-6 bg-gray-50/50 dark:bg-black transition-colors duration-300">
