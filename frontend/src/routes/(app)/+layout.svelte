...
>
  ...
  <div class="flex-1 flex flex-col overflow-hidden">
    <!-- Topbar -->
    <header
      class="h-16 border-b border-gray-200 bg-white/60 dark:border-gray-800 dark:bg-gray-900/40 backdrop-blur flex items-center justify-between px-6 shrink-0"
    >
      <div></div>
      <div class="flex items-center space-x-4">
        {#if backendStatus === "connected"}
          <span
            class="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-medium tracking-tight"
            >API CONNECTED</span
          >
        {/if}
        <button
    ...
    <!-- Page Content -->
    <main
      class="flex-1 overflow-y-auto p-6 bg-gray-50/50 dark:bg-black transition-colors duration-300"
    >
